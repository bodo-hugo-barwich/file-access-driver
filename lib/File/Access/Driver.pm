  ...
  if($sflnm ne '')
  {
    ...
    if($self->Exists)
    {
      ...
      else  #Change File Name failed
      {
        ...
        $self->{'_error_message'} .= "File '" . $self->{'_directory_name'}
          . $self->{'_file_name'} . "': Change File Name failed with [$irs]!"
          . "Message: '$!'\n";
...
sub setFileTime
{
  ...
  if($self->Exists)
  {
    ...
    $irs = utime $self->{'_file_access_time'}, $itmfl
      , $self->{'_directory_name'} . $self->{'_file_name'} ;
    ...
    if($irs < 1)
    {
      ...
      $self->{'_error_message'} .= "File '" . $self->{'_directory_name'}
        . $self->{'_file_name'} . "': Set File Time failed with [$irs]!"
        . "Message: '$!'\n";
      $self->{'_error_code'} = 1 if($self->{'_error_code'} < 1);
...
my $fopenrFile = sub
{
  ...
  if($self->$fisOpen())
  {
    ...
    $self->$fcloseFile() if($self->$fisWritable());
  } #if($self->$fisOpen())
  ...
  unless($self->$fisOpen())
  {
    ...
    {
      #Open the File
      $irs = open $self->{'_file'}, '<', $self->{'_directory_name'}
        . $self->{'_file_name'};
      ...
      {
        $self->{'_error_message'} .= "File '" . $self->{'_directory_name'} . $self->{'_file_name'} . "': "
          . "Open Read failed!\n"
          . "Message: '$!'\n";
        $self->{'_error_code'} = 1 if($self->{'_error_code'} < 1);
...
my $fopenwFile = sub
{
  ...
  unless($self->$fisWritable())
  {
    $self->$fcloseFile() if($self->$fisOpen());
    ...
    if($self->{"_directory_name"} ne "")
    {
      unless(-d $self->{"_directory_name"})
      {
        ...
        $self->{"_report"} .= "Directory '" . $self->{"_directory_name"} . "': "
        ...
        if($@)
        {
          $self->{"_error_message"} .= "Directory '" . $self->{"_directory_name"} . "': "
    ...
    if($irs)
    {
      if($self->{"_file_name"} ne "")
      {
        ...
        {
          $self->{"_error_message"} .= "File '" . $self->{"_directory_name"}
            . $self->{"_file_name"} . "': Open Write failed!\n"
            . "Message: '$!'\n";
          $self->{"_error_code"} = 1 if($self->{"_error_code"} < 1);
  ...
  unless($self->$fisAppendable())
  {
    ...
    if($irs)
    {
      if($self->{"_file_name"} ne "")
      {
        ...
        {
          $self->{"_error_message"} .= "File '" . $self->{"_directory_name"}
            . $self->{"_file_name"} . "': Open Append failed!\n"
            . "Message: '$!'\n";
          $self->{"_error_code"} = 1 if($self->{"_error_code"} < 1);
...
sub Read
{
  ...
  $self->$fopenrFile() if($self->$fisWritable());

  $self->$fopenrFile() unless($self->$fisOpen());
  ...
  if($self->$fisOpen())
  {
    ...
    do  #while($irdcnt);
    {
      ...
      {
        $self->{"_error_message"} .= "File '" . $self->{"_directory_name"}
          . $self->{"_file_name"} . "': Read File failed with [" . (0 + $!) . "]!!"
          . "Message: '$!'\n";
        $self->{"_error_code"} = 1 if($self->{"_error_code"} < 1);
      } #if(defined $irdcnt)
    }
    ...
    if($irs)
    {
      ...
      {
        $self->{"_error_message"} .= "File '" . $self->{"_directory_name"}
          . $self->{"_file_name"} . "': File Attributes failed!\n";
      } #if(scalar(@arrflstt) > 0)
    } #if($irs)
...
sub Write
{
  ...
  if($self->$fisWritable())
  {
    ...
    if(defined $iwrtcnt)
    {
      ...
      {
        $self->{"_error_message"} .= "File '" . $self->{"_directory_name"}
          . $self->{"_file_name"} . "': '$iwrtcnt' from '$icntntlen' Bytes written.\n";
      }
    }
    else  #An Error has ocurred
    {
      $self->{"_error_message"} .= "File '" . $self->{"_directory_name"}
        . $self->{"_file_name"} . "': File Write failed!\n"
        . "Message: '$!'\n";
      $self->{"_error_code"} = 1 if($self->{"_error_code"} < 1);
    } #if(defined $iwrtcnt)
...
sub Delete
{
  ...
  if($self->Exists)
  {
    ...
    {
      $self->{"_error_message"} .= "File '" . $self->{"_directory_name"}
        . $self->{"_file_name"} . "': Delete File failed with [" . (0 + $!) . "]!"
        . "Message: '$!'\n";
      $self->{"_error_code"} = 1 if($self->{"_error_code"} < 1);
